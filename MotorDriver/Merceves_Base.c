...
// 68 alternating magnets. Counting both edges (rise + fall) gives
// 68 edges per revolution on a bipolar hall sensor.
// Adjust if your sensor is omnipolar (would be 136 with both edges).
...
bool rpm_timer_callback(struct repeating_timer *t) {
    ...
    // Write to globals (read by SPI handler on core 1)
    left_rpm  = new_left;
    right_rpm = new_right;
