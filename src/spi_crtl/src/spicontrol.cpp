...
public:
    SPINode() : Node("spi_node") {
        ...
        // 150 Hz timer drives SPI transfers independently of ROS messages.
        // This ensures RPM data is read at a steady rate even when no
        // new drive commands arrive.
...
private:
    ...
    void do_spi_transfer() {
        ...
        // Validate start byte
        if (rx_data[0] != 0xAA) {
            // Misaligned — discard this frame. Both sides send 0xAA as
            // byte 0 every frame, so alignment self-corrects next time.
